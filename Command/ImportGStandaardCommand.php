...
class ImportGStandaardCommand extends ContainerAwareCommand
{
	...
	protected function updateCacheTables(InputInterface $input, OutputInterface $output) {
		...
		\Propel::getConnection()->query("
				...
				,	IF(t3.`naam_item_50_posities` = 'STUK', sam.hoeveelheid_werkzame_stof, sam.hoeveelheid_werkzame_stof*a.`hoeveelheid_per_deelverpakking`)
				,	a.`inkoopprijs`
				...
				LEFT JOIN `gs_declaratietabel_dure_geneesmiddelen` as dwg ON (hpk.`handelsproduktkode` = dwg.`handelsproduktkode` AND dwg.`zorgactiviteit_voldoet_aan_beleidsregel` = 1 AND (dwg.`einddatum` = '0000-00-00' OR dwg.`einddatum` > CURDATE()))
